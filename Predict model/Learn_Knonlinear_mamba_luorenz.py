...
def read_lorenz_npy_original_shape(file_path):
    data = np.load(file_path)
    return data

def train(env_name,train_steps = 100000,suffix="",Ktrain_samples=50000):   
    Ktrain_samples = Ktrain_samples
    Ktest_samples = 20000#20000
    Ksteps = 50
    Kbatch_size = 100
    gamma = 0.8
    X_original_shape = read_lorenz_dataset_original_shape('utility_LSPN/lorenz_data.csv')
    # X_original_shape = read_lorenz_npy_original_shape('utility_LSPN/luorenz_data100000.npy')
    ...
    best_loss = 1000.0
    best_state_dict = {}
    logdir = "DATA/Mamba_data_raw/"+suffix+"/KNonlinearmamba_"+"luorenz"+"samples{}".format(Ktrain_samples)
...
if __name__ == "__main__":
    env_names = ["luorenz"]#"MountainCarContinuous-v0","MountainCarContinuous-v0",,"CartPole-v1","MountainCarContinuous-v0","Pendulum-v1"
    for i in env_names:
        parser = argparse.ArgumentParser()
        parser.add_argument("--env",type=str,default=i)
        parser.add_argument("--suffix",type=str,default="mamba_testluorenz1")
