...
def train(env_name,train_steps = 100000,suffix="",Ktrain_samples=3000):   
    Ktrain_samples = Ktrain_samples
    Ktest_samples = 1000#20000
    Ksteps = 50
    Kbatch_size = 100
    gamma = 0.8
    X_original_shape = read_rossler_dataset_original_shape('utility_LSPN/filtered_rossler_data.npy')
    ...
    best_loss = 1000.0
    best_state_dict = {}
    logdir = "DATA/Mamba_data_raw/"+suffix+"/KNonlinearmamba_"+"rossler"+"samples{}".format(Ktrain_samples)
    if not os.path.exists( "DATA/Mamba_data_raw/"+suffix):
        os.makedirs( "DATA/Mamba_data_raw/"+suffix)
    if not os.path.exists(logdir):
        os.makedirs(logdir)
    writer = SummaryWriter(log_dir=logdir)
    for i in trange(train_steps, desc="rossler", unit="epoches"):
...
if __name__ == "__main__":
    env_names = ["rossler"]#"MountainCarContinuous-v0","MountainCarContinuous-v0",,"CartPole-v1","MountainCarContinuous-v0","Pendulum-v1"
    for i in env_names:
        parser = argparse.ArgumentParser()
        parser.add_argument("--env",type=str,default=i)
        parser.add_argument("--suffix",type=str,default="mamba_testrossler1")
